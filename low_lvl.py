...
def tuple_test (tuple_t):
    test_list=len(list(zip(*tuple_t[::-1])))
    return test_list
...
def putMouseOnConfirm() :
    x = 410
    y = 515
    autopy.mouse.move(x,y)

def putMouseOnEndTurn() :
    x = 740
    y = 310
    autopy.mouse.move(x,y)    

##if going first
def putMouseOnStartingManaCardOne() :
    x = 165
    y = 245
    autopy.mouse.move(x,y)

def putMouseOnStartingManaCardTwo() :
    x = 360
    y = 245
    autopy.mouse.move(x,y)

def putMouseOnStartingManaCardThree() :
    x = 555
    y = 245
    autopy.mouse.move(x,y)

def putMouseOnHeroPower(x, y) :
    autopy.mouse.smooth_move(x, y)

##if going second  TODO
#def fourPutMouseOnStartingManaCardOne() :
#    x = 165
#    y = 245
#    autopy.mouse.move(x,y)
#
#def fourPutMouseOnStartingManaCardTwo() :
#    x = 360
#    y = 245
#    autopy.mouse.move(x,y)
#
#def fourPutMouseOnStartingManaCardThree() :
#    x = 555
#    y = 245
#    autopy.mouse.move(x,y)    
...
def calibrate() :
    putMouseOnStart()
    print("mouse is on start")
    time.sleep(1)
    putMouseOnConfirm()
    print("mouse is on confirm")
    time.sleep(1)
    putMouseOnStartingManaCardOne()
    print("mouse is on card 1 mana")
    time.sleep(1)
    putMouseOnStartingManaCardTwo()
    print("mouse is on card 2 mana")
    time.sleep(1)
    putMouseOnStartingManaCardThree()
    print("mouse is on card 3 mana")
    time.sleep(1)
    putMouseOnHeroPower()
    print("mouse is on hero power")
    time.sleep(1)
    putMouseOnEndTurn()
    print("mouse is on end turn")
    time.sleep(1)            


def findone(img_template,zone,accu,rect) :
    autopy.bitmap.capture_screen(rect).save('src/screengrab.png')#screen capture
...
#zone : 0 allboard / 1 ennemy / 2 self /3 hand
    ...
    if zone == 0 : #allboard /!\/!\/!\/!\/!\/!\/!\ Completement optimisable -> ne garder que le cas 5
        ...
        offset = [120,450]#
        screen = screen[450:830,120:808]
        #cv2.imshow('try',screen)
        #cv2.waitKey(0)
        #cv2.destroyAllWindows()
